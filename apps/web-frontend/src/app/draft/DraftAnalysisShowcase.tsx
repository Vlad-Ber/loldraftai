                ...
                <tr key={i} className="border-b">
                  ...
                  <td
                    className={`p-2 text-right ${
                      champion_impact[i] > 0 ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    ({(champion_impact[i] * 100).toFixed(1)}%)
                  </td>
                  <td className="p-2 text-right">
                    {gold_diff_15min[i] > 0
                      ? Math.abs(gold_diff_15min[i]).toFixed(0)
                      : ""}
                  </td>
                  <td className="p-2 text-right">
                    {gold_diff_15min[i] < 0
                  ...
                  <td
                    className={`p-2 text-right ${
                      champion_impact[i + 5] > 0
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
                    ({(champion_impact[i + 5] * 100).toFixed(1)}%)
                  </td>
                  <td className="p-2 text-right">
