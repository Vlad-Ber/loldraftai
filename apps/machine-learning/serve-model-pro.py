...
print(f"Creating ONNX Runtime session from {PRO_ONNX_MODEL_PATH}")
...
try:
    onnx_session = ort.InferenceSession(str(PRO_ONNX_MODEL_PATH), session_options)
...
async def get_metadata(api_key: str = Depends(verify_api_key)):
    ...
    model_timestamp = datetime.fromtimestamp(os.path.getmtime(str(PRO_ONNX_MODEL_PATH)))
